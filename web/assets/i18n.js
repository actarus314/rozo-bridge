...
const I18N={
  fr:{
    ...
    newVersion:v=>`nouvelle version ${v} dispo ↗`,
    ...
    pCurveExplain:'<ul style="margin:0 0 0 -2px"><li>Points = mesures live (dryrun, identiques pour tous les appId).</li><li><b>Balayage paresseux</b> : lancé à l\'<b>ouverture de la page Doc</b>, seulement pour les montants <b>≤ plafond de liquidité</b> (+ un point exact au plafond) — sert uniquement à <b>ce graphe</b>, pas au devis.</li><li>Lignes verticales = plafond de liquidité <b>live</b> (= Available de l\'API Rozo, ≈ solde on-chain du hub) ; chaque courbe s\'<b>arrête à son plafond</b>.</li><li>Au-delà du plafond, un envoi unique est refusé (fractionne).</li><li>Les petits montants montent vers ~0,50 % (plancher 0,01 € + arrondi cent).</li><li>La génération donne le chiffre engageant.</li></ul>',
    ...
    pWhyRange:"Une fourchette n'apparaît que sur le sens <b>Stellar→Base</b>.<br>Là, créer un intent <b>réserve l'Available</b> du hub : empiler des tranches le draine, donc chaque tranche suivante coûte plus.<br>Le découpage s'y lit alors en <b>trois valeurs</b> :<ul><li><b>min</b> — N devis <b>dryrun réels</b> (un par taille de tranche T/n), sommés. C'est le frais exact si chaque tranche partait à hub plein, sans drainage. Meilleur cas, <b>exact</b>.</li><li><b>max</b> — les tranches créées en série, chaque intent drainant le hub. Pire cas, <b>estimé</b> depuis la surface mesurée (cap 0,50 %), <b>collé au réel ≤0,04 €</b>.</li><li><b>coût probable</b> — affiché entre les deux, même surface, ancré sur le min exact.</li></ul>En pratique, les N créations partent en <b>parallèle</b>.<br>Le serveur partage le snapshot par <b>vagues</b>.<br>Le réalisé est donc une <b>variable aléatoire</b> entre min et max — proche du min quand il y a peu de tranches.<br>Sur <b>Base→Stellar</b>, créer un intent <b>fige le frais mais ne réserve pas l'Available</b>.<br>Sans drainage, <b>chaque tranche a une valeur unique</b> (min = max) : ni fourchette, ni coût probable.<br>La <b>génération donne le chiffre engageant</b>.",
    ...
    modelLi4:"Routes : le niveau base% <b>et</b> la montée diffèrent (B→S ~0,09 &lt; S→B ~0,12 %) → <b>une surface par route</b>, mesurée séparément pour projeter max et coût probable. Validé : <b>max ≤0,04 €</b> / 16 séries réelles — jamais optimiste ; le <b>min</b> est un devis <b>dryrun réel</b> (exact, pas une projection).",
    ...
    genBatchStale:`<span class="mut">Devis en cours de recalcul — réessaie dans un instant.</span>`,
    chunkFeeError:`frais indisponible (réseau)`,
    retryBtn:`réessayer`,
    ...
    reservedNote:(dk,recv,hub)=>dk==="S2B"?`<b>Liquidité réservée</b> ${recv} EURC sur le ${hub} · frais <b>figés</b> jusqu'à expiration (~10 min). Non signé → self-heal, aucun fonds bloqué.`:`Frais <b>figés</b> jusqu'à expiration (~10 min) · <b>aucune liquidité réservée</b> sur ce sens. Non signé → rien à faire, aucun fonds bloqué.`,
  ...
  en:{
    ...
    pWhyRange:"A range only appears on the <b>Stellar→Base</b> direction.<br>There, creating an intent <b>reserves the hub's Available</b>: stacking chunks drains it, so each next chunk is dearer.<br>The split then reads as <b>three values</b>:<ul><li><b>min</b> — N real <b>dryrun quotes</b> (one per chunk size T/n), summed. It's the exact fee if every chunk went out at a full hub, no drain. Best case, <b>exact</b>.</li><li><b>max</b> — chunks created in series, each intent draining the hub. Worst case, <b>estimated</b> from the measured surface (0.50% cap), <b>within ≤0.04€ of reality</b>.</li><li><b>likely cost</b> — shown between the two, same surface, anchored on the exact min.</li></ul>In practice, the N creations fire in <b>parallel</b>.<br>The server shares the snapshot in <b>waves</b>.<br>The realized cost is therefore a <b>random variable</b> between min and max — close to min when there are few chunks.<br>On <b>Base→Stellar</b>, creating an intent <b>freezes the fee but doesn't reserve the Available</b>.<br>With no drain, <b>each chunk has a single value</b> (min = max): no range, no likely cost.<br><b>Generating gives the binding figure</b>.",
    ...
    modelLi4:"Routes: the base% level <b>and</b> the climb differ (B→S ~0.09 &lt; S→B ~0.12%) → <b>one surface per route</b>, measured separately to project max and likely cost. Validated: <b>max ≤0.04€</b> / 16 real series — never optimistic; the <b>min</b> is a real <b>dryrun quote</b> (exact, not a projection).",
    ...
    genBatchStale:`<span class="mut">Quote is repricing — try again in a moment.</span>`,
    chunkFeeError:`fee unavailable (network)`,
    retryBtn:`retry`,
    ...
    reservedNote:(dk,recv,hub)=>dk==="S2B"?`<b>Liquidity reserved</b> ${recv} EURC on the ${hub} · fees <b>frozen</b> until expiry (~10 min). Unsigned → self-heals, no funds locked.`:`Fees <b>frozen</b> until expiry (~10 min) · <b>no liquidity reserved</b> on this direction. Unsigned → nothing to do, no funds locked.`,
