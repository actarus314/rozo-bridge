...
> it's provided **as-is, with no warranty**: always double-check amounts and destination
> addresses yourself before signing anything. Use at your own risk.
...
- **Best-split calculator** — splitting a large transfer into smaller chunks can lower the
  ...
  captures ≥90% of the possible saving, so you're not signing 10 transactions to save
  a few cents.
- **Live liquidity, both directions** — the hub balance is a thin, moving float, not a deep
  ...
  Base RPC), so you know before you send whether your amount will even go through.
- **Generate, sign, track** — turns the recommended split into real Rozo intents, lets you
  sign each chunk with your own wallet (any injected EVM wallet on Base, any Stellar
  Wallets Kit wallet), and tracks delivery live until every chunk lands.
- **Zero backend, zero API key** — a static page that talks directly to Rozo's public API
  and to Stellar Horizon / Base RPC from your browser. Nothing to deploy, nothing to trust
  beyond what you can read in `assets/app.js`.
...
python3 serve.py
```
Opens the page locally and serves `assets/`. Requires only Python 3 — no install step,
no build, no `.env`. The server stops itself when you close the tab.

## Command-line quote

Prefer a terminal? `CLI/rozo-quote.sh` gets you the same exact quotes and liquidity
checks without opening a browser — see [`CLI/README.md`](CLI/README.md).
...
> détient jamais de fonds. Mais il est fourni **tel quel, sans garantie** : vérifie
> toujours toi-même les montants et adresses de destination avant de signer quoi que ce
> soit. À utiliser à tes risques.
...
- **Générer, signer, suivre** — transforme le découpage recommandé en vrais intents Rozo,
  laisse signer chaque tranche avec ton propre wallet (tout wallet EVM injecté sur Base,
  tout wallet compatible Stellar Wallets Kit), et suit la livraison en direct jusqu'à ce
  que chaque tranche arrive.
- **Zéro backend, zéro clé API** — une page statique qui parle directement à l'API
  publique de Rozo et à Stellar Horizon / RPC Base depuis ton navigateur. Rien à déployer,
  rien à croire au-delà de ce que tu peux lire dans `assets/app.js`.
...
python3 serve.py
```
Ouvre la page localement et sert `assets/`. Nécessite seulement Python 3 — aucune
installation, aucun build, aucun `.env`. Le serveur s'arrête tout seul à la fermeture de
l'onglet.

## Devis en ligne de commande

Tu préfères le terminal ? `CLI/rozo-quote.sh` donne les mêmes devis exacts et vérifications
